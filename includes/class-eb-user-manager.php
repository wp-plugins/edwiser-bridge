...
class EB_User_Manager {
	 ...
	 */
	public function user_course_synchronization_handler( $sync_options = array(), $user_id_to_sync = '' ) {
		...
		if ( $connected['success'] == 1 ) {
			...
			if( is_numeric( $user_id_to_sync ) ) {
				$all_users = $wpdb->get_results( "SELECT user_id, meta_value AS moodle_user_id FROM {$wpdb->prefix}usermeta WHERE user_id = ".$user_id_to_sync." AND meta_key = 'moodle_user_id' AND meta_value IS NOT NULL", ARRAY_A );
			} else {
			...
			foreach ( $all_users as $key => $value ) {
				...
				if ( isset( $sync_options['eb_synchronize_user_courses'] ) && $sync_options['eb_synchronize_user_courses'] == 1 ) {

					// get user's enrolled courses from moodle
					$moodle_user_courses = EB()->course_manager()->get_moodle_courses( $value['moodle_user_id'] );
					...
					if ( $moodle_user_courses['success'] == 1 ) {
						foreach ( $moodle_user_courses['response_data'] as $course_data ) {
							...
							$existing_course_id = EB()->course_manager()->is_course_presynced( $course_data->id );
							...
							if ( is_numeric( $existing_course_id ) ) {
								...
								EB()->enrollment_manager()->update_enrollment_record_wordpress( $args ); // update enrollment records
								EB()->logger()->add( 'user', 'New course enrolled, User ID: '.$value['user_id'].' Course ID: '.$existing_course_id ); // add user log
							}
						}
					} else {
						// push user's id to separate array if there is a problem in fetching his/her courses from moodle.
						$response_array['user_with_error'][] = "<p style='float:left; clear:left;'><strong>User ID: </strong>".$value['user_id']."</p></br>";
					}

					/**
					 * In this block we are unenrolling user course if a user is unenrolled from those course on moodle
					 */
					$existing_enrolled_courses = $wpdb->get_results( "SELECT course_id from {$wpdb->prefix}moodle_enrollment WHERE user_id = ".$value['user_id'], ARRAY_A ); // get user's existing enrollment record from wordpress DB
					$notenrolled_courses = array();

					foreach ( $existing_enrolled_courses as $existing_course ) {
					...
					if ( is_array( $notenrolled_courses ) && !empty( $notenrolled_courses ) ) {
						...
						EB()->enrollment_manager()->update_enrollment_record_wordpress( $args );
					}
			...
			EB()->logger()->add( 'user', "Connection problem in synchronization, Response:". print_r( $connected, true ) ); // add connection log
		}
	 ...
	 */
	public function create_wordpress_user( $email, $firstname, $lastname ) {
		...
		if ( email_exists( $email ) ) {
			return new WP_Error( 'registration-error', __( 'An account is already registered with your email address. Please login.', 'eb-textdomain' ) );
		}
		...
		$wp_user_data = apply_filters( 'eb_new_user_data', array(
				'user_login' => $username,
				'user_pass'  => $password,
				'user_email' => $email,
				'role'       => 'subscriber'
		...
		if ( is_wp_error( $user_id ) ) {
			return new WP_Error( 'registration-error', '<strong>' . __( 'ERROR', 'eb-textdomain' ) . '</strong>: ' . __( 'Couldn&#8217;t register you&hellip; please contact us if you continue to have problems.', 'eb-textdomain' ) );
		}
		...
		$created = isset( $moodle_user['user_created'] )?$moodle_user['user_created']:0;
	 ...
	 * @return boolean           returns true / false  [ return false in case of connection failure ]
	 */
	public function is_moodle_username_available( $username ) {
		global $wpdb;

		EB()->logger()->add( 'user', 'Checking if username exists....' ); // add to user log
	...
	public function get_moodle_user( $user_email ) {
		...
		} elseif ( $response['success'] == 1 && is_array( $response['response_data'] ) && !empty( $response['response_data'] ) ) {
	 ...
	 *       );
	 *
	 * @param integer $update    set update = 1 if you want to update an existing user on moodle.
	...
	public function create_moodle_user( $user_data, $update = 0 ) {
		...
		EB()->logger()->add( 'user', 'Start creating/updating moodle user, Updating: '.$update ); // add user log
		...
		// EB()->logger()->add( 'user', 'Create/Update moodle user response: '.serialize( $response ) ); // add user log
	...
	public function link_moodle_user( $user ) {
		...
		if ( empty( $moodle_user_id ) ) {
			...
			if ( isset( $moodle_user['user_exists'] ) && $moodle_user['user_exists'] == 1 && is_object( $moodle_user['user_data'] ) ) {
				...
				// sync courses of an individual user when an exisintg moodle user is linked to wordpress account with same email
				$this->user_course_synchronization_handler( array( 'eb_synchronize_user_courses' => 1 ), $user->ID );
		...
		if (  $current_screen->base == 'users' ) { ?>
            <script type="text/javascript">
              jQuery(document).ready(function() {
                jQuery('<option>').val('link_moodle').text('<?php _e( 'Link Moodle Account', 'eb-textdomain' )?>').appendTo("select[name='action']");
                jQuery('<option>').val('link_moodle').text('<?php _e( 'Link Moodle Account', 'eb-textdomain' )?>').appendTo("select[name='action2']");
                jQuery('<option>').val('unlink_moodle').text('<?php _e( 'Unlink Moodle Account', 'eb-textdomain' )?>').appendTo("select[name='action']");
                jQuery('<option>').val('unlink_moodle').text('<?php _e( 'Unlink Moodle Account', 'eb-textdomain' )?>').appendTo("select[name='action2']");
              });
            </script>
        <?php
		}
	 ...
	 * perform the action
	 *
	 * This does not delete users account from moodle on unlink, but it can create a new moodle account on 'link account' action.
	...
	public function link_user_bulk_actions_handler() {
		...
		case 'link_moodle':
			...
			if ( is_array( $users ) ) {
				...
				$sendback = add_query_arg( array( 'linked' => $linked ), $sendback );
			}
		...
		case 'unlink_moodle':
			...
			if ( is_array( $users ) ) {
				...
				// build the redirect url
				$sendback = add_query_arg( array( 'unlinked' => $unlinked ), $sendback );
			}
	...
	public function link_user_bulk_actions_notices() {
		...
		if ( $pagenow == 'users.php' ) {
			...
			if ( isset( $_REQUEST['unlinked'] ) && $_REQUEST['unlinked'] == 1 ) {
				...
				$message = sprintf( __( '%s Users Unlinked.', 'eb-textdomain' ), number_format_i18n( $_REQUEST['unlinked'] ) );
	 ...
	 */
	public function password_update( $user_id, $user ) {
		...
		EB()->logger()->add( 'user', 'Password update initiated..... ' ); // add user log
	...
	public function display_users_enrolled_courses( $user ) {
        ...
        <select name="enroll_course">
            ...
            <?php foreach ( $notenrolled_courses as $course ) {
	...
	public function update_courses_on_profile_update( $user_id ) {
		...
		if ( is_numeric( $moodle_user_id ) ) {
			...
			$enroll_course    = isset( $_POST['enroll_course'] )?$_POST['enroll_course']:'';
			$unenroll_course  = isset( $_POST['unenroll_course'] )?$_POST['unenroll_course']:'';
			...
			if ( is_numeric( $enroll_course ) ) {
				...
				EB()->enrollment_manager()->update_user_course_enrollment( $args ); // enroll user to course
			}
	...
	public function delete_enrollment_records_on_user_deletion( $user_id ) {
		...
		EB()->logger()->add( 'user', "Enrollment records of user ID: {$user_id} are deleted." );  // add user log
	}
}
